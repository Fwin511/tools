...
支持使用 `_as_` 前缀定义字段别名，使用 `_only_` 前缀触发精确匹配，避免与系统字段或保留字冲突。

### 使用场景

当 URL 参数名与系统保留字段冲突时，可以使用别名：

```php
// GET /api/tasks?taskResult._as_submit_staff_id=100

// 等同于搜索 taskResult.submit_staff_id 字段
Task::query()
    ->with('taskResult')
    ->autoFilter()
    ->get();
...
- **普通字段**: `_as_field_name` → 转换为 `field_name`
- **关联字段**: `relation._as_field_name` → 转换为 `relation.field_name`
- **精确字段**: `_only_field_name` → 转换为 `field_name`，字符串走 `=` 查询
- **关联精确字段**: `relation._only_field_name` → 转换为 `relation.field_name`，字符串走 `=` 查询

### 示例

```php
// 1. 普通字段别名
// GET /api/users?_as_status=active
// 实际搜索字段: status

// 2. 关联字段别名
// GET /api/orders?user._as_name=张三&orderItems._as_quantity=5
// 实际搜索字段: user.name, orderItems.quantity
...
// 5. 字符串精确查询
// GET /api/goods?_only_goods_code=0421-1811
// 实际搜索字段: goods_code，使用 '=' 精确匹配
```

### 注意事项

- 以单个下划线 `_` 开头的参数（除了 `_as_`、`_only_`）会被认为是系统参数并自动排除
- 以 `_as_` 开头会被识别为别名字段；以 `_only_` 开头会被识别为精确匹配字段
- 支持普通字段和关联表字段的别名
